...
describe(@"Github Resource", ^{
    ...
    context(@"を初期化するとき", ^{
        ...
        it(@"は、APIパスを引数に受け取り、認可情報を付加したURLを構築する", ^{
            ...
            NSURL *url = githubResorce.resourceUrl;
            NSString *urlString = [NSString stringWithFormat:@"https://api.github.com/user?access_token=%@", kDebugAccessToken];
        ...
        it(@"は、クエリ付きのAPIパスを引数に受け取り、認可情報を付加したURLを構築する", ^{
            ...
            NSURL *url = githubResorce.resourceUrl;
            NSString *urlString = [NSString stringWithFormat:@"https://api.github.com/user?page=1&access_token=%@", kDebugAccessToken];
        });
    });
});
